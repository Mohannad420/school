...
if(isset($_POST['submit'])){
    ...
    if(empty($name)){
        ...
        $error[] = "الاسم طويل جدآ";
            ...
            $stm = "SELECT name FROM students WHERE name = '$name'";
            $q=$conn->prepare($stm);
            $q->execute();
            $data=$q->fetch();
                ...
                if(empty($error)){
                    ...
                    $stm = "INSERT INTO students VALUES('','$number','$name','$teacher',''.)";
...
if(isset($_POST['edit'])){
    ...
    // VALIDATE EMAIL
            ...
            if(empty($error)){
                ...
                $id = $_SESSION['id'];
                $stm = "UPDATE `students` SET  `grade` = '$grade' WHERE `students`.`name` = '$name'";
                $conn->prepare($stm)->execute();
...
<div class="container-fluid d-flex justify-content-center row row-cols-5">
            ...
            <table class="  table table-dark table-striped   ">
                ...
                <tbody>
                    ...
                    <?php
                        ...
                        $nono = $_SESSION['ok'];
                        $conn = mysqli_connect('localhost','root','','test');
                        $query = "Select * from grade WHERE cousename = '$nono' ";
                        $stmt = $conn->prepare($query);
                        $stmt->execute();
                        $resultSet = $stmt->get_result();
