...
function foo() 
{
    ...
    $stm = "SELECT * FROM teachers WHERE id = '$id'";
    $q=$conn->prepare($stm);
    $q->execute();
    $data=$q->fetch();
    if($data){
        ...
        $dell = "DELETE FROM teachers WHERE id = $id ";
    ...
    if(empty($name)){
        ...
        if(empty($emai)){
            ...
            if($data){
                ...
                // $sstm = "INSERT INTO teachers VALUES('','$name','$email','0000')";
                ...
                $nn = $name;
                $ee = $email;
                // $stm = "UPDATE `teachers` SET `Name` = '$name', `Email` = '$email' WHERE `users`.`ID` = '$id'";
                ...
                if(empty($error)){
                    ...
                    $stm = "INSERT INTO teachers VALUES('','$name','$email','$phone','$password','$file')";
                    $conn->prepare($stm)->execute();
    ...
    // VALIDATE EMAIL
        if(empty($emai)){
            ...
            if(empty($error)){
                ...
                $id = $_SESSION['id'];
                $stm = "UPDATE `teachers` SET `name` = '$name', `email` = '$email' ,`phone` = '$phone' , `password` = '$password' , `img` = '$file'  WHERE `teachers`.`id` = '$id'";
                $conn->prepare($stm)->execute();
...
<body style="background-image: linear-gradient(#77bbff, hotpink);  min-height: 100vh;">
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container-fluid">
            ...
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                <a class="nav-link active border-bottom border-danger border-top fw-bold" aria-current="page" href="admin1.php">Teachers</a>
                </li>
                <li class="nav-item">
    ...
    <div class="container-fluid row row-cols-2 d-flex justify-content-center m-0 p-5 "  >    
                ...
                <!-- FORM -->
                    <form class=" py-4 me-2 mt-5 bg-dark text-light col-4 row row-cols-1    " style="height:30%;" action="admin1.php" method="post" >
                        <div class="mb-3 ">
                        ...
                        if (count($datas)> 0) { 
                            ...
                            foreach($datas as $key=>$val){
                                ...
                                <td >
                                    ...
                                    <a href="admin1.php?id= '. $val['id'].' " type="button" class="btn btn-sm btn-primary"  >
                                    Edit
                                    </a>
                                    ...
                                    <div class="dropdown dropend d-inline">
                                        ...
                                        <ul class="dropdown-menu p-0 m-0 w-25 ">
                                        <li class=""><a href="admin1.php?id='. $val['id'].'&del=1" class=" text-center dropdown-item btn btn-sm btn-Warning " >Confirm Deletion</a></li>
